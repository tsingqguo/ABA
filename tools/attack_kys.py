...
from attacker_kys import attacker
...
img_num = 5
imlist=[]
test_mode = 'testall'   #   debug    testall  visionall  test_2  选择特定帧攻击  transfer_siam  transfer_dimp
attack_mode = 'guo_speed'    #     guo  nor_blur    whitebox     obj_speed   guo_speed_dirflow
attack_bool =  True
tran_path = '/workspace/ABA/experiments/siamrpn_mobilev2_l234_dwxcorr/'    #  siamrpn_mobilev2_l234_dwxcorr   siamrpn_r50_l234_dwxcorr
...
def main():
    ...
    ack_dimp = attacker_dimp()
    if test_mode == 'transfer' or test_mode == 'transfer_dimp':  # tracker_att 是生成攻击样本的tracker
    ...
    if args.dataset in ['VOT2016', 'VOT2018', 'VOT2019'] : #and not os.path.exists(os.path.join('attack', args.dataset, model_name)):  #,'baseline'
            ...
            for idx, (img, gt_bbox) in enumerate(video):
                ...
                if idx == frame_counter:
                    ...
                    if idx %5 == 0 and attack_bool and test_mode == 'testall':
                        ...
                        outputs = tracker.track(img, x_crop = out_ack['adv_x_crop'], coord = out_ack['coord'],v=True)
                    ...
                    elif idx %5 == 0 and attack_bool and test_mode == 'transfer_dimp':
                        X1=img.copy()
                        #print(X0.shape)
                        ...
                        out_ack = ack_dimp(img0=X0,img1=X1,mode = 'guo_crop_speed' , model_attacked = tracker_att, speed = True , crop = [x0_patches,x1_patches])
                        #print(img_attack.size())
                        #save_torchimg(out_ack['adv_x_crop'],'trans')
                        outputs = tracker.track(img,  x_crop  = out_ack['adv_x_crop'], coord = pos )
                        ...
                        outputs = tracker.track(img,  x_crop  = resize_blur ,coord = pos )
        ...
        #for v_idx, video in enumerate(dataset.reverse()):
        for v_idx in range(len(dataset)):
            video = dataset[v_idx]
            #video = dataset[99-v_idx]
            #print(video.name)
