...
class attacker(nn.Module):
    ...
    def attack_once(self, tracker, data):
        ...
        scores_dimp, _ , _  = tracker.get_response_prediction(backbone_feat, scores,needgrad =True)

        motion_feat = tracker.net.get_motion_feat(backbone_feat)

        prev_motion_feat, prev_motion_state_vector, prev_test_patch, prev_label, box_patch = tracker.prev_state_handler.get_data()

        box_patch = box_patch.to(prev_motion_feat.device)
        box_c = box_patch[0:2] + 0.5 * box_patch[2:]

        box_c_max = tracker.img_sample_sz[0] * (0.5 + 1.0 / tracker.params.search_area_scale)
        box_c_min = tracker.img_sample_sz[0] * (0.5 - 1.0 / tracker.params.search_area_scale)

        if prev_motion_state_vector is not None and tracker.params.get('move_feat_to_center', False) and not \
        ...
        elif tracker.params.get('prev_feat_remove_subpixel_shift', False) and prev_motion_state_vector is not None:
            box_c_feat = box_c / 16.0
            box_c_feat_round = box_c_feat.round() + 0.5 * tracker.net.predictor.fix_coordinate_shift
            feat_trans = (box_c_feat_round - box_c_feat).view(-1, 2) / tracker.output_sz.view(-1, 2).to(box_c_feat.device)
        ...
        if tracker.output_window is not None and tracker.params.get('apply_window_to_dimp_score', True):
            scores_dimp_win = scores_dimp * tracker.output_window
            print(scores_dimp.size())
            
            vis.heatmap(scores_dimp[0][0])
        ...
        predictor_input = {'dimp_score_cur': scores_dimp_win,
                           ...
                           'anno_prev': tracker.prev_anno.to(tracker.params.device),
                           'state_prev': prev_motion_state_vector}

        window_fn = tracker.output_window
        
        
        resp_output = tracker.net.predictor.predict_response(predictor_input, dimp_thresh=tracker.params.get('dimp_threshold', None),
                                                              output_window=window_fn)
        scores = F.relu(resp_output['response'])
        '''
        #print(test_x.size() , scores.size())
        ...
        total_loss = cls_loss + 0.00001 * reg_loss
    ...
    def forward(self,img0,img1,mode ='guo',model_attacked = None,tracker =None, speed = True, crop = None):
        if mode == 'whitebox' or mode =='nor_blur':
            ...
            #生成GT 
            backbone_feat,  patch_coords  , _= tracker.extract_backbone_features(label_in, tracker.get_centered_sample_pos(),
            ...
            scores_dimp = tracker.classify_target(test_x)  

            
            scores_gt = scores_dimp
            #scores_gt, motion_feat, new_state_vector = tracker.get_response_prediction(backbone_feat, scores_dimp)
            #print(test_x.size(),scores_gt.size()) 
            ...
            mix_patches, patch_coords = sample_patch_multiscale(out_img, tracker.get_centered_sample_pos(),
                                                                      ...
                                                                      max_scale_change = tracker.params.get('patch_max_scale_change', None))
            im_sz = mix_patches.size(2)
            #print(mix_patches.size())
            
            
            
            alpha_samll = torch.ones(2*divnum,1,im_sz, im_sz).cuda()/(2*divnum)
            alpha_org = torch.ones(2*divnum,1,im_sz, im_sz).cuda()/(2*divnum)
            ...
            #vis.image(flow_full[:,0,:,:])
            #vis.image(flow_full[:,1,:,:])
            x1_in =  X1[0, :, intPaddingTop:intPaddingTop+intHeight, intPaddingLeft: intPaddingLeft+intWidth].unsqueeze(0).cuda()*255     #.permute(1,2,0)
            x0_in =  X0[0, :, intPaddingTop:intPaddingTop+intHeight, intPaddingLeft: intPaddingLeft+intWidth].unsqueeze(0).cuda()*255 
            flow_full = flow_full.permute(0,2,3,1)[0, intPaddingTop:intPaddingTop+intHeight, intPaddingLeft: intPaddingLeft+intWidth,:]
            #print(flow_full.size())   
            #剪切光流致search大小
            flow_x = flow_full[:,:,0].repeat(1,3,1,1)  
            flow_y = flow_full[:,:,1].repeat(1,3,1,1)  
            #print(flow_x.size())
            
            flow_xsamll, _ = sample_patch_multiscale(flow_x, tracker.get_centered_sample_pos(),
                                                                      tracker.target_scale * tracker.params.scale_factors,
                                                                      tracker.img_sample_sz, getattr(tracker.params, 'border_mode', 'replicate'),
                                                                      max_scale_change = tracker.params.get('patch_max_scale_change', None))
                                                                 
            flow_ysamll, _ = sample_patch_multiscale(flow_y, tracker.get_centered_sample_pos(),
                                                                      tracker.target_scale * tracker.params.scale_factors,
                                                                      tracker.img_sample_sz, getattr(tracker.params, 'border_mode', 'replicate'),
                                                                      max_scale_change = tracker.params.get('patch_max_scale_change', None))   
            flow_full_crop = torch.stack((flow_xsamll[0][0],flow_ysamll[0][0])).permute(1,2,0).unsqueeze(0)                                                                                                                 
            #print(flow_full_crop.size())
            
            x0_small, _ = sample_patch_multiscale(x0_in, tracker.get_centered_sample_pos(),
                                                                      tracker.target_scale * tracker.params.scale_factors,
                                                                      tracker.img_sample_sz, getattr(tracker.params, 'border_mode', 'replicate'),
                                                                      max_scale_change = tracker.params.get('patch_max_scale_change', None))
                                                                 
            x1_small, patch_coords = sample_patch_multiscale(x1_in, tracker.get_centered_sample_pos(),
                                                                      tracker.target_scale * tracker.params.scale_factors,
                                                                      tracker.img_sample_sz, getattr(tracker.params, 'border_mode', 'replicate'),
                                                                      max_scale_change = tracker.params.get('patch_max_scale_change', None)) 
            if crop is not None :
                x0_small = crop[0]
                x1_small = crop[1]
            #print(x1_in.size(),x1_small.size())
            #生成GT 和adv_gt
            backbone_feat = tracker.net.extract_backbone(x1_small)
            test_x = tracker.get_classification_features(backbone_feat)
            scores_gt = tracker.classify_target(test_x)    #kys 不用生成最后的heatmap ，BP classify_target 的结果即可攻击
            ...
            while m < max_iterations:
                ...
                scores = scores[..., :-1, :-1].contiguous()
                #vis.heatmap(scores[0][0])
                #scores , _ , _ = tracker.get_response_prediction(backbone_feat, scores,True)
                #print(scores.size(),adv_cls.size())
                ...
                if MIFGSM:
                    ...
                    x_grad = torch.sign(x_grad)
                    #print(x_grad.sum())
                    alpha_samll = (alpha_samll + epsilon * x_grad).detach()
                    if speed :
                        sx = (sx + epsilon_s * torch.sign(sx_grad)).detach()
                        sy = (sy + epsilon_s * torch.sign(sy_grad)).detach()
        ...
        elif mode =='guo_crop_speed':  # 直接把剪切好的image 输入光流
            tracker = model_attacked
            #print(img1.shape)
            X0 = crop[0]
            X1 = crop[1]
            '''
            img0 = self.img_to_torch(img0).unsqueeze(0)  
            img1 = self.img_to_torch(img1).unsqueeze(0)  

            X0, _ = sample_patch_multiscale(img0, tracker.get_centered_sample_pos(),
                                                                      tracker.target_scale * tracker.params.scale_factors,
                                                                      tracker.img_sample_sz, getattr(tracker.params, 'border_mode', 'replicate'),
                                                                      max_scale_change = tracker.params.get('patch_max_scale_change', None))
                                                                 
            X1, patch_coords = sample_patch_multiscale(img1, tracker.get_centered_sample_pos(),
                                                                      tracker.target_scale * tracker.params.scale_factors,
                                                                      tracker.img_sample_sz, getattr(tracker.params, 'border_mode', 'replicate'),
                                                                      max_scale_change = tracker.params.get('patch_max_scale_change', None)) 
            '''                                                          
            ...
            cur_offset_input = torch.cat((X0_big, X1_big), dim=1).cuda()
            #print(cur_offset_input.size())
            ...
            #x1_in =  X1[0, :, intPaddingTop:intPaddingTop+intHeight, intPaddingLeft: intPaddingLeft+intWidth].unsqueeze(0).cuda()*255     #.permute(1,2,0)
            #x0_in =  X0[0, :, intPaddingTop:intPaddingTop+intHeight, intPaddingLeft: intPaddingLeft+intWidth].unsqueeze(0).cuda()*255 
            flow_full_crop = flow_full.permute(0,2,3,1)[0, intPaddingTop:intPaddingTop+intHeight, intPaddingLeft: intPaddingLeft+intWidth,:].unsqueeze(0)  
            #print(flow_full.size())   
            
            #vis.heatmap(flow_full[:,:,0])
            #vis.heatmap(flow_full[:,:,1])                                                                                                                
            #print(flow_full_crop.size())
            ...
            #print(x1_in.size(),x1_small.size())
            #生成GT 和adv_gt
            backbone_feat = tracker.net.extract_backbone(X1)
            test_x = tracker.get_classification_features(backbone_feat)
            scores_gt = tracker.classify_target(test_x)    #kys 不用生成最后的heatmap ，BP classify_target 的结果即可攻击
    ...
    def get_adv_lab(self,scores_gt ):
        ...
        adv_no = True 
        while adv_no:
            yp = random.choice((1, -1)) * random.randint(4,ss//2)
            xp = random.choice((1, -1)) * random.randint(4,ss//2)

            if 1<xx+xp<ss-1 and 1<yy+yp<ss-1:
                break 
