...
class SiameseTracker(BaseTracker):
    def get_subwindow(self, im, pos, model_sz, original_sz, avg_chans):
        ...
        if any([top_pad, bottom_pad, left_pad, right_pad]):
            ...
            im_patch = im[int(context_ymin):int(context_ymax + 1),
                          int(context_xmin):int(context_xmax + 1), :]
        #print(context_ymin,context_ymax,context_xmin,context_xmax)
        self.ymin = int(context_ymin)
        self.ymax = int(context_ymax)
        self.xmin = int(context_xmin)
        self.xmax = int(context_xmax)
    ...
    def get_chengorg(self , img,crop):
        
        yy = int(self.ymax-self.ymin)
        xx = int(self.xmax-self.xmin)
        im_patch = crop.cpu()
        im_patch = torch.squeeze(im_patch)
        im_patch = im_patch.detach().numpy()
        im_patch = im_patch.astype(np.uint8)
        ...
        im_patch = cv2.resize(im_patch,(yy+2,xx+2),3)
        print(yy,xx)
        cv2.imwrite('/workspace/ABA/adv_in_org.jpg', im_patch)
        img[self.ymin-1:self.ymax+1,self.xmin-1:self.xmax+1,:] = im_patch
