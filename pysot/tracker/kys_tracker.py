...
class KYSTracker(pyBaseTracker):
    ...
    def track(self, image, x_crop = None,coord = None,v = False):
        self.debug_info = {}
        ...
        if  x_crop is not None:  
            backbone_feat = self.net.extract_backbone(x_crop)  
            im_patches =  x_crop 
            sample_coords = coord
        ...
        #print(scores_fused.size(),motion_feat.size(), new_state_vector.size())   #torch.Size([1, 1, 18, 18]) torch.Size([1, 1024, 18, 18]) torch.Size([1, 8, 18, 18])
    
        #vis.heatmap(scores_fused[0][0])
        if  v : 
        ...
        #vis.heatmap(self.output_window[0])

        out = {'bbox': new_state.tolist()}
    ...
    def get_response_prediction(self, backbone_feat, scores_dimp, needgrad=False):
        motion_feat = self.get_motion_features(backbone_feat)   # 提取layer3 特征输入  输出torch.Size([1, 1024, 18, 18]) 只是做size变换，无卷积等操作
        
        prev_motion_feat, prev_motion_state_vector, prev_test_patch, prev_label, box_patch = self.prev_state_handler.get_data() # prev_motion_feat([1, 1024, 18, 18])
        '''
        self.info_dict = {'frame_number': frame_number, 'feat': feat, 'state': state, 'im': im, 'bb': bb,
                          'label': label, 'bb_patch': bb_patch}
        '''
        ...
        box_c = box_patch[0:2] + 0.5 * box_patch[2:] #前一帧 bbox ,提取bbox 中心位置
        #print(prev_label.size(), box_patch)  # prev_label.Size([1, 1, 18, 18])
        
        #                     18*16
        box_c_max = self.img_sample_sz[0] * (0.5 + 1.0 / self.params.search_area_scale) #search_area_scale = 5
        box_c_min = self.img_sample_sz[0] * (0.5 - 1.0 / self.params.search_area_scale)
        #print(box_c,box_c_max,box_c_min)
        #vis.heatmap(prev_motion_feat[0][12])
        ...
        if self.output_window is not None and self.params.get('apply_window_to_dimp_score', True):
            scores_dimp_win = scores_dimp * self.output_window #汉宁窗，类似二维高斯
            #print('window')
            #input()
        else:
            scores_dimp_win = scores_dimp
        #print(self.output_window.size())
        #vis.heatmap(self.output_window[0])
        #print(self.prev_anno,prev_label)
        
        predictor_input = {'dimp_score_cur': scores_dimp_win,  #当前帧map
                           'label_prev': prev_label,
                           'feat1': prev_motion_feat, 'feat2': motion_feat, #前后两帧 backbone feature
                           'anno_prev': self.prev_anno.to(self.params.device),
                           'state_prev': prev_motion_state_vector}  #前一帧state
    ...
    def update_memory(self, sample_x: TensorList, target_box, learning_rate=None):
        ...
        #print('update_memory',self.training_samples.size(),replace_ind)
    ...
    def update_classifier(self, train_x, target_box, learning_rate=None, scores=None):
        ...
        #print(self.params.get('train_sample_interval', 1))
