...
def adv_attack_template_S(img_tensor, GAN, target_sz=(127,127)):
    ...
    with torch.no_grad():
        ...
        return img_adv
def adv_attack_search(img_tensor, imgpre_tensor , GAN,search_sz=(255,255)):
    '''adversarial attack to search region'''
    '''input: pytorch tensor(0,255) ---> output: pytorch tensor(0,255)'''
    '''step1: Normalization'''
    img_tensor = normalize(img_tensor)
    imgpre_tensor = normalize(imgpre_tensor)
    #print(img_tensor.size())
    
    '''step2: pass to G'''
    with torch.no_grad():
        GAN.search_clean1 = img_tensor
        GAN.search_clean0 = imgpre_tensor
        GAN.num_search = img_tensor.size(0)
        GAN.forward(img2blur =[img_tensor*127+127,imgpre_tensor*127+127])
    img_adv = GAN.blur_adv 
    #print(img_adv)
    #save_torchimg(img_adv,'advblur_test')
