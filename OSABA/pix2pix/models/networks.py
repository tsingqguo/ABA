...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02, gpu_ids=[]):
    ...
    elif netG == 'cheng_unet_256':
        net = cheng_UnetGenerator(input_nc, output_nc, 6, ngf, norm_layer=norm_layer, use_dropout=use_dropout)
    else:
...
class UnetSkipConnectionBlock(nn.Module):
    ...
    def __init__(self, outer_nc, inner_nc, input_nc=None,
        ...
        if outermost:
            upconv_w = nn.ConvTranspose2d(inner_nc * 2, outer_nc,
                                        kernel_size=4, stride=2,
                                        padding=1)
            upsam_f = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)    
            conv3 =   nn.Conv2d(inner_nc * 2, outer_nc, kernel_size=3, padding=1)
            conv1_f = nn.Conv2d(outer_nc, 2, kernel_size=1, padding=0)
           
          
            down = [downconv]
            up = [uprelu, upconv_w, nn.Tanh()]
            model = down + [submodule] #+ up   uprelu ,
            modelf = [ upsam_f , conv3 , conv1_f, downrelu]
            self.fin_w  = nn.Sequential(*up)
            self.fin_f  = nn.Sequential(*modelf)
        elif innermost:
    ...
    def forward(self, x):
        if self.outermost:
            x_base = self.model(x)
            x_w = self.fin_w(x_base)
            x_f = self.fin_f(x_base)
            x_f = F.avg_pool2d(x_f,kernel_size=3,stride=1,padding=1) 
            return x_f , x_w , x_base
