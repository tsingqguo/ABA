...
class GsearchL2500dimpModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['G_L2', 'fool' , 'W_L2', 'adv']
    ...
    def set_input(self, input):
        ...
        """
        self.template = input[0].squeeze(0).numpy()#.squeeze(0).cuda()  # pytorch tensor, shape=(1,3,127,127)
        self.bbox = input[3][0].numpy()
        self.search_clean255_0 = input[1].squeeze(0)
        self.search_clean255_1 = input[2].squeeze(0)
        #print(self.template.shape,self.bbox)
        self.search_sharp0 = torch.zeros(7,1,3,288,288)
        self.search_sharp1 = torch.zeros(7,1,3,288,288)
        with torch.no_grad():
            #print(self.search_clean255_0.size())
            self.siam.tracker.init(self.template, self.bbox)
            for q in range(self.search_clean255_0.size(0)):
                self.search_sharp0[q], cood = sample_patch_multiscale(self.search_clean255_0[q].unsqueeze(0), self.siam.tracker.get_centered_sample_pos(),
                                                                      self.siam.tracker.target_scale * self.siam.tracker.params.scale_factors,
                                                                      self.siam.tracker.img_sample_sz, getattr(self.siam.tracker.params, 'border_mode', 'replicate')) 
                                                                     
                self.search_sharp1[q], coord = sample_patch_multiscale(self.search_clean255_1[q].unsqueeze(0), self.siam.tracker.get_centered_sample_pos(),
                                                                      self.siam.tracker.target_scale * self.siam.tracker.params.scale_factors,
                                                                      self.siam.tracker.img_sample_sz, getattr(self.siam.tracker.params, 'border_mode', 'replicate')) 
                
            
            self.search_blur_n , self.mix_img = self.blur.get_blur_norm(self.search_sharp0.squeeze(1).cuda(),self.search_sharp1.squeeze(1).cuda())
            #print(self.search_blur_n.size() , self.mix_img.size())
            self.coord = coord
            #print(self.search_clean255.size(),self.template.size())
            self.score_maps_clean = self.siam.get_heat_map(self.search_blur_n,coord)#(5HWN,),with softmax

        #print(self.score_maps_clean,self.score_maps_clean.size())
        #vis.heatmap(self.score_maps_clean[0][0])
        
        self.search_clean0 = normalize(self.search_sharp0.squeeze(1).cuda())
      
        self.search_clean1 = normalize(self.search_sharp1.squeeze(1).cuda())
        self.num_search = self.search_clean1.size(0)
        # print('clean image shape:',self.init_frame_clean.size())


    def forward(self,target_sz=(288,288),img2blur= None):
        ...
        if img2blur is not None :
            ...
            #print(self.search_clean255_0.size())
            self.search_sharp0 = self.search_clean255_0.unsqueeze(0) 
            self.search_sharp1 = self.search_clean255_1.unsqueeze(0) 
        ...
        #for t in range(2):
            ...
            #vis.heatmap(flow_1[0][t])
            #vis.heatmap(flow_2[0][t])
            #input()
        ...
        #print(self.W.size(),flow_x.size(),target_sz)
        
        self.blur_adv , self.loss_flow = self.blur(self.search_sharp0.squeeze(1).cuda(),self.search_sharp1.squeeze(1).cuda(),self.W,flow_x,flow_y)
        #print(self.blur_adv)
    ...
    def backward_G(self):
        ...
        self.loss_G_L2 = self.criterionL2(self.blur_adv_11, self.search_blur_n_11) * self.opt.lambda_L2 * 0.01
        ...
        if num_attention > 0:
            if myadvloss:
                ...
                self.loss_W_L2 = torch.norm(self.W-w_norm, 2) * 0.000001
                #adv_lab =  get_advlabel(self.score_maps_clean)
                adv_lab = torch.zeros(1,1,19,19).cuda()
                adv_lab[:,:,13,4] = 1
                adv_lab[:,:,3,4] = 1
                adv_lab[:,:,4,13] = 1
                adv_lab[:,:,13,14] = 1
            ...
            self.loss_adv = torch.norm((self.score_maps_adv - adv_lab),2) * 300
            #self.loss_fool_cheng = torch.mean(torch.clamp(score_map_adv_rand[:, 1] - score_map_adv_rand[:, 0], min=self.margin)) * self.weight
            self.loss_fool = torch.mean(torch.clamp(score_map_adv_att, min=self.margin)) * self.weight
            # combine loss and calculate gradients
            self.loss_G = self.loss_G_L2 + self.loss_fool  +  self.loss_W_L2 + self.loss_adv
        else:
    ...
    def optimize_parameters(self):
        ...
        self.score_maps_adv  = self.siam.get_heat_map(self.search_adv255 , self.coord)#(5HWN,2),without softmax
        '''backward pass'''
        # update G
    ...
    def forward(self,X0,X1,W,sx,sy):
        ...
        for n in range(W.size(0)):
            ...
            for i in range(divnum):
                ...
                theta_fw_x =    (sx[n,:i+1,:,:].sum(0))     * self.flow[n].unsqueeze(0)[:,:,:,0]     
            ...
            warp_mix = torch.cat((warped01,warped01),0)
            #print(warp_mix.size())
