...
def save_torchimg(img,name):
    ...
    cv2.imwrite('/cheng/AttackTracker/compare_img/{}.png'.format(name), save_im)
def main():
    # load config
    ...
    for v_idx, video in enumerate(dataset):
        ...
        for idx, (img, gt_bbox) in enumerate(video):
            ...
            if idx == 0:
                ...
                ss = time.time()
                x_crop, pert_true, prev_perts, weights, img_  , org_search = attacker.attack(tracker, img, t_prev_perts, t_weights, APTS, OPTICAL_FLOW,ADAPT,Enable_same_prev=args.enable_same_pert)
                if idx%5 == 0 and save_search:
                        #print(outputs['x_crop_adv'].size(),outputs['x_crop'].size())
                        save_torchimg(x_crop,'adv_x_crop_{}_{}'.format(video.name,idx))
                        save_torchimg(org_search,'frame1_{}_{}'.format(video.name,idx))
                        #print((x_crop-org_search).sum())
                st  = time.time()
                attack_time += st-ss
                # import visdom
                # vis = visdom.Visdom()
