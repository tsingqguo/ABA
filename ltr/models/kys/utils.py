...
class CenterShiftFeatures(nn.Module):
    ...
    def forward(self, feat, anno):
        anno = anno.view(-1, 4)  #xywh
        c_x = (anno[:, 0] + anno[:, 2] * 0.5) / self.feature_stride  #feat 的中心坐标
...
class CenterShiftFeatures_center(nn.Module):
    def __init__(self, feature_stride):
        super().__init__()
        self.feature_stride = feature_stride
    ...
    def forward(self, feat, anno):
        anno = anno.view(-1, 2)
        c_x = (anno[:, 0] ) #/ self.feature_stride
        c_y = (anno[:, 1] ) #/ self.feature_stride
