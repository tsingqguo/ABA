...
class ResponsePredictor(nn.Module):
    def __init__(self, state_dim=8, representation_predictor_dims=(64, 32), gru_ksz=3,
                 prev_max_pool_ksz=1, conf_measure='max', dimp_thresh=None , scale_map =0.08,vis_map=True):
        super().__init__()
        ...
        self.representation_predictor2 = nn.Sequential(*representation_predictor_list)
        ...
        self.response_predictor2 = nn.Sequential(
            conv_block(in_dim, 1, kernel_size=3, stride=1, padding=1, batch_norm=False, relu=False),
            nn.Sigmoid())

        self.state_predictor = ConvGRUCell(8, state_dim, gru_ksz)
    ...
    def forward(self, cost_volume, state_prev, dimp_score_cur, init_label=None, dimp_thresh=None,
        ...
        auxiliary_outputs = {}
        dimp_score_cur = dimp_score_cur* self.scale_map
        ...
        if self.conf_measure != 'none':
            ...
            predictor_input = torch.cat(
                    (propagated_h, dimp_score_cur[:,0,:,:].view(num_sequences, 1, *dimp_score_cur.shape[-2:]),
                     propagation_conf), dim=1)   #  
            predictor_input2 = torch.cat(
                    (propagated_h, dimp_score_cur[:,1,:,:].view(num_sequences, 1, *dimp_score_cur.shape[-2:]),
                     propagation_conf), dim=1)
        else:
            predictor_input = torch.cat(
                    (propagated_h, dimp_score_cur[:,0,:,:].view(num_sequences, 1, *dimp_score_cur.shape[-2:])), dim=1)   
            predictor_input2 = torch.cat(
                    (propagated_h, dimp_score_cur[:,1,:,:].view(num_sequences, 1, *dimp_score_cur.shape[-2:])), dim=1)
        resp_representation = self.representation_predictor(predictor_input)
        resp_representation2 = self.representation_predictor2(predictor_input2)
        fused_prediction = self.response_predictor(resp_representation)   #*0.4
        fused_prediction2 = self.response_predictor2(resp_representation2) #*0.4
        score_mix = dimp_score_cur.sum(1).unsqueeze(1)
        auxiliary_outputs['score_mix'] = score_mix 
        #print(propagated_h.size())  init_label
        
        #print(fused_prediction.size(),dimp_score_cur.size())
        if dimp_thresh is not None:
            fused_prediction = fused_prediction * (dimp_score_cur[:,0,:,:].view(num_sequences, 1, *dimp_score_cur.shape[-2:]) > dimp_thresh).float()
            fused_prediction2 = fused_prediction2 * (dimp_score_cur[:,1,:,:].view(num_sequences, 1, *dimp_score_cur.shape[-2:]) > dimp_thresh).float()
        self.idsample +=1
        if (self.idsample %400==0 or self.idsample==-1 ) and self.vis_map:
            #vis.heatmap(propagation_conf[0][0])
            #vis.heatmap(score_mix[0][0])
            vis.heatmap(init_label[0])
            vis.heatmap(propagated_h[0][0])
            vis.heatmap(propagated_h[0][2])
            vis.heatmap(propagated_h[0][4])
            vis.heatmap(dimp_score_cur[0][0])
            vis.heatmap(dimp_score_cur[0][1])
            vis.heatmap(fused_prediction[0][0])
            vis.heatmap(fused_prediction2[0][0])
            print('look')
            input()
        real_map1 = fused_prediction #* dimp_score_cur[:,0,:,:].unsqueeze(1) 
        real_map2 = fused_prediction2 #* dimp_score_cur[:,1,:,:].unsqueeze(1) 
        auxiliary_outputs['fused_score_orig'] = [fused_prediction,fused_prediction2]
        '''vis.heatmap(dimp_score_cur[0][0])
        vis.heatmap(dimp_score_cur[0][1])
        vis.heatmap(fused_prediction[0][0])
        vis.heatmap(fused_prediction2[0][0])'''
        ...
        #fused_prediction = score_mix * fused_prediction
        #print(dimp_score_cur.size())
        scores_cat = torch.cat((dimp_score_cur, fused_prediction,fused_prediction2), dim=1) #4
        ...
        #print(scores_cat.size())
        #state_gru_input = scores_cat
        state_gru_input = torch.cat((scores_cat, scores_cat_pool), dim=1)  #8
        ...
        return [fused_prediction / self.scale_map,fused_prediction2/self.scale_map], state_new, auxiliary_outputs