...
class ResponsePredictor(nn.Module):
    def __init__(self, state_dim=8, representation_predictor_dims=(64, 32), gru_ksz=3,
                 prev_max_pool_ksz=1, conf_measure='max', dimp_thresh=0.02):
        super().__init__()
        ...
        in_dim = state_dim  + 5 + (conf_measure != 'none')
        ...
        '''self.con11 = conv_block(state_dim+1, 16, kernel_size=3, stride=1, padding=1,batch_norm=False, relu=True)
        self.con12 = conv_block(16, 1, kernel_size=3, stride=1, padding=1,batch_norm=False, relu=False)

        self.con21 = conv_block(state_dim+1, 16, kernel_size=3, stride=1, padding=1,batch_norm=False, relu=True)
        self.con22 = conv_block(16, 1, kernel_size=3, stride=1, padding=1,batch_norm=False, relu=False)

        self.con31 = conv_block(2, 16, kernel_size=3, stride=1, padding=1,batch_norm=False, relu=True)
        self.con32 = conv_block(16, 1, kernel_size=3, stride=1, padding=1,batch_norm=False, relu=False)'''

        self.state_predictor = ConvGRUCell(20, state_dim, gru_ksz)  #state  update
    ...
    def forward(self, cost_volume, state_prev, dimp_score_cur, init_label=None, dimp_thresh=None,
        ...
        cost_volume = cost_volume.view(-1, 1, feat_sz[0], feat_sz[1])  # n*h*w x 1 x h x w
        cost_volume_p1 = self.cost_volume_proc1(cost_volume).view(-1, feat_sz[0] * feat_sz[1])
        #print(cost_volume_p1.size())  #torch.Size([324, 324])
        
        cost_volume_p1 = F.softmax(cost_volume_p1, dim=1)
        #print(cost_volume_p1)
        ...
        #print(state_prev_ndhw.shape)
        #for s in range(8):
            #vis.heatmap(state_prev_ndhw[0][s])
        
        is_target = self.is_target_predictor(state_prev_ndhw)  #2层卷积
        auxiliary_outputs['is_target'] = is_target
        #print(state_prev_ndhw.size())  #  torch.Size([1, 8, 18, 18])
        ...
        #print(state_prev_nhwd.size())   #  torch.Size([1, 324, 8, 18, 18])
        ...
        elif self.conf_measure == 'entropy': #这里
            
            propagation_conf = propagation_weight_norm.view(num_sequences, -1, feat_sz[0], feat_sz[1])
            #print(propagation_conf.size())   #torch.Size([1, 324, 18, 18])
            propagation_conf = -(propagation_conf * (propagation_conf + 1e-4).log()).sum(dim=1)  #论文公式 5
        #print(propagation_conf.size())   # torch.Size([1, 18, 18])
        ...
        #print(propagated_h.size()) #  torch.Size([1, 8, 18, 18])
        #propagated_h = state_prev_ndhw
        ...
        if self.conf_measure != 'none':
            ...
            #predictor_input = propagated_h

            predictor_input = torch.cat(
                    (propagated_h, dimp_score_cur.view(num_sequences, 5, *dimp_score_cur.shape[-2:]),
                     propagation_conf ), dim=1)
        else:
            
            predictor_input = propagated_h
            #predictor_input = torch.cat(
                #(propagated_h, dimp_score_cur), dim=1)


        '''o1 = self.con11(torch.cat((propagated_h,      dimp_score_cur.view(num_sequences, 1, *dimp_score_cur.shape[-2:]) ),dim=1))
        o1 = torch.sigmoid(self.con12(o1))
        o2 = self.con21(torch.cat((propagation_conf,  propagated_h     ),dim=1))
        o2 = torch.sigmoid(self.con22(o2))
        o3 = self.con31(torch.cat((propagation_conf,   dimp_score_cur.view(num_sequences, 1, *dimp_score_cur.shape[-2:])   ), dim=1))
        o3 = torch.sigmoid(self.con32(o3))

        #print(o1.size(),o2.size(),o3.size())
        
        fused_prediction = (o1+o2+o3)/3
        
        auxiliary_outputs['ee_loss'] = self.Ensemble_Entropy(fused_prediction)
        auxiliary_outputs['pred_list'] = [o1,o2,o3]'''

        #print(predictor_input.size())  #torch.Size([1, 10, 18, 18])
        #for s in range(9):
            #vis.heatmap(predictor_input[0][s])
        ...
        #print( dimp_score_cur.size())
        #print(fused_prediction.size())
        vis.heatmap(fused_prediction[0][0])
        #vis.heatmap(dimp_score_cur[0][0])
        #input()
        fused_prediction = 1 * fused_prediction * dimp_score_cur#.view(num_sequences, 1, *dimp_score_cur.shape[-2:])
        #print(fused_prediction.size())   #torch.Size([1, 1, 18, 18])
        #vis.heatmap(fused_prediction[0][0])
        ...
        if output_window is not None:
            ...
            #print(dimp_thresh)
        #vis.heatmap(fused_prediction[0][0])
        #print(dimp_score_cur.size(), fused_prediction.size())
        scores_cat = torch.cat((dimp_score_cur, fused_prediction), dim=1)
        #print(F.adaptive_max_pool2d(scores_cat, 1).size())
        ...
        #print(scores_cat_pool.size()) 
        
        state_gru_input = torch.cat((scores_cat, scores_cat_pool), dim=1)
        #print(state_gru_input.size(),propagated_h.size())
    ...
    def Ensemble_Entropy(self, pred_sum, num_model=3):
        
       
        Ensemble = -( pred_sum * ( pred_sum + 1e-4).log()).sum()/pred_sum.size(0)
        #print(Ensemble.size())
        
        return Ensemble.item()