...
class KYSProcessing(BaseProcessing):
    ...
    def __call__(self, data: TensorDict):
        ...
        #cv2.imwrite('/cheng/pytracking-master/ltr/img/orig.jpg', data['test_images'][0])
       
        
        #fff , _ = prutils.crop_and_resize(data['test_images'][0].copy(),None,data['test_anno'][0].int().numpy(),125)
        #cv2.imwrite('/cheng/pytracking-master/ltr/img/temple.jpg', fff)
        for s in ['train', 'test']:
            ...
            #cv2.imwrite('/cheng/pytracking-master/ltr/img/search.jpg', crops[0])
            #print(boxes[0])
            # Add transforms
            data[s + '_images'], data[s + '_anno'] = self.transform[s](image=crops, bbox=boxes, joint=False)
        
        #print(data[s + '_anno'][0])
...
class STARKProcessing(BaseProcessing):
    ...
    def __init__(self, search_area_factor, output_sz, center_jitter_factor, scale_jitter_factor,
                 mode='sequence', settings=None, *args, **kwargs):
        """
        args:
            search_area_factor - The size of the search region  relative to the target size.
            output_sz - An integer, denoting the size to which the search region is resized. The search region is always
